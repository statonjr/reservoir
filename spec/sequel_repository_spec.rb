...
describe Reservoir::SequelRepository do
  ...
  describe "#find_by_key" do
    ...
    it 'returns the right entity by key' do
      ...
      repo.save(request)
      result = repo.find_by_key('dealership-a1-b2-c3')
      expect(result.first[:key]).to eq('dealership-a1-b2-c3')
    end
    it 'should not return keys with :deleted => true' do
      deleted_bmw = {:kind => :dealership, :name => 'Hendrick BMW Northlake', :key => 'dealership-98-76-qw', :deleted => true}
      repo.save(deleted_bmw)
      result = repo.find_by_key('dealership-98-76-qw')
